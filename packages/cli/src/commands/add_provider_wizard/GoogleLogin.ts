    ...
    return new Promise((resolve, reject) => {
        ...
        const server = createServer((req, res) => {
            ...
            if (urlObj.pathname === "/callback") {
                const token = urlObj.searchParams.get("token") || "mock-oauth-google-token-123";
                
                const html = renderHtmlTemplate(
                    "Connected Successfully!",
                    "🦖",
                    "#4caf50",
                    "Google Gemini OAuth token has been received and saved locally in your multiplexus CLI."
                );
